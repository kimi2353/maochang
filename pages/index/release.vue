  ...
  <div class="contents">
    ...
    <block v-if="name == '动态'||name == '打赏'|| name == '投票' || name == '悬赏' || name == '奖品'">
      ...
      <view class="release_text">
        ...
        <view class="uni-textarea">
          <textarea v-model="contentValue" @focus="funHandle" @blur="blurFun()"
            placeholder-style="color:#9B9DA9;font-size: 15px;" placeholder="分享美好瞬间..." />
        </view>
        <view class="release_img">
          <view class="show_img" v-for="(item,index) in imgOneArr" :style="(index+1)%3 ==0?'':'margin-right: 56rpx;'"
            :key="index">
    ...
    <block v-if="name == '动态'|| name == '投票'">
      ...
      <view class="vote" v-if="voteNone">
        ...
        <view class="vote_img">
          <image @click="remove_value(99)" v-if="vote_value.length <=1" src="../../static/img/reduce.png" mode="">
          </image>
      ...
      <view class="vote_content" v-for="(item, index) in vote_content">
        <view class="voteContent_num">
          ...
          <input v-model="vote_value[index].content" @input="voteHandle" type="text" value=""
            placeholder-style="color:#9B9DA9" placeholder="请输入" />
        </view>
        <image :src="vote_value[index].image" mode="" @click="voteImageFun(index)"></image>
        <image v-if="vote_content.length > 1" @click="remove_value(index)"
          style="width: 36rpx; height: 36rpx; margin: 16rpx 0 0 0 ;" src="../../static/img/reduce.png" mode=""></image>
      </view>
      <!-- 时间 -->
    ...
    <block v-if="name == '跳蚤市场'">
      ...
      <view class="vote_boxTwo flea">
        <input v-model="fleaValue" type="number" :maxlength="30" placeholder-style="font-size: 15px;color: #9B9DA9;"
          placeholder="输入价格" />
      </view>
      <view class="vote_boxOne">
      ...
      <view class="vote_boxTwo Three">
        <textarea v-model="contentValue" :maxlength="200" placeholder-style="font-size: 15px;color: #9B9DA9;"
          placeholder="简单描述一下你的商品信息" />
        <view class="vote_boxTwo_text" style="margin-bottom: 60rpx;">{{Product_description.length}}/200</view>
        <view class="release_img box">
          <!-- 上传选择的图片显示 用到循环 -->
          <view class="show_img boxOne" v-for="(item,index) in imgOneArr"
            :style="(index+1)%3 ==0?'':'margin-right: 56rpx;'" :key="index">
            <image class="img" :src="item" mode="aspectFill">
            </image>
            <image @click="remove_imgOne(item)" class="img_remove" src="../../static/img/imgRemove.png" mode=""></image>
          </view>
      ...
      <view class="label_text">
        <view class="label_content" :style="type_id == item.id?'background-color: #52BCBC;color: #fff;':''"
          v-for="(item,index) in labelArr" :key="index" @click="labelHandle(item.id)">
          {{item.type_name}}
        </view>
      </view>
    </block>
    <block v-if="name == '提问'">
      <view class="vote_boxTwo Three">
        <textarea style="height: 300rpx; margin-bottom: 40rpx;" v-model="Product_description"
          placeholder-style="font-size: 15px;color: #9B9DA9;" placeholder="分享你的瞬间" />
...
<script>
  ...
  export default {
    ...
    onShow() {
      ...
      this.hot_topic_id = uni.getStorageSync("hot_topic_id")
    },
    //下拉刷新
    onPullDownRefresh() {
    ...
    onUnload() {
      ...
      uni.setStorageSync("hot_topic_id", '')
    ...
    methods: {
      ...
      //获取跳蚤市场的分类
      marketType() {
        this.$REQ(this.$api.marketType, {}, 'POST').then(res => {
      ...
      getPublish() {
        ...
        console.log({
          ...
          to_user: "", //@的用户
          to_topic: this.hot_topic_id, //选择的话题
          position: '', //位置信息
          images: images, //上传的图片
      ...
      set_number() {
        if (this.vote_content.length < 10 || this.vote_value.length < 10) {
          ...
          this.vote_value.push({
            content: '',
            image: '../../static/img/toupiao2.png'
          })
      ...
      voteShow(a) {
        ...
        if (a == 2) {
          ...
          return
          uni.navigateTo({
            url: "/pages/index/contacts?name=" + that.name + "&number=" + that.number
          })
        }
        //跳转到热门话题
        if (a == 3) {
