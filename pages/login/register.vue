    ...
    <view class="back">
      <view class="login_submit">
        <view class="submit_titile">
          <view>邮 箱：</view>
          <view @click="sendOut" class="code" v-if="msgCode === 1"
            >发送验证码</view
          >
          <view class="hasBeenSent" v-if="msgCode === 2"
            >{{ second }}秒重新发送</view
          >
          <view class="code" v-if="msgCode === 3" @click="sendOut"
            >重新发送验证码</view
          >
        </view>
        <input
      ...
      <view class="login_submit">
        <span class="submit_titile"> 确 认 密 码： </span>
        <input
          class="submit_input"
          v-model="newPwd"
          type="password"
